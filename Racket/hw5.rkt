  ...
  (cond [(var? e)
		...
		[(mlet? e)
		 (eval-under-env (mlet-body e) (cons (cons (mlet-var e)
											 ...
											 env))]
		[(fst? e)
		 (let ([f (eval-under-env (fst-e e) env)])
		   (if (apair? f)
			   (apair-e1 f)
			   (error "Expected MUPL pair")))]
		 ...
		 (let ([clo (eval-under-env (call-funexp e) env)]
		   ...
		   (if (closure? clo)
			   (let ([f (closure-fun clo)]
					 [newe (closure-env clo)])
				 (if (fun-nameopt f)
					 (eval-under-env
					   (fun-body f)
					   (cons
						 (cons (fun-nameopt f) clo)
						 (cons
						   (cons (fun-formal f) arg)
						   (if (null? env)
							   newe
							   (cons env newe)))))
					 (eval-under-env
					   (fun-body f)
						 (cons
						   ...
						   (if (null? env)
							   newe
							   (cons env newe))))))
			   (error "Tried to call something not a function")))]
		[(closure? e)
		 e]
		[(aunit? e)
		 e]
...
(define mupl-mapAddN
  (mlet "map" mupl-map
		(fun #f "i"
			 (fun #f "lst" (call (call (var "map") (fun #f "x"
														(add (var "i") (var "x"))))
								 (var "lst"))))))
