          ...
          case eval_prog(e1, env) of
             ...
             | LineSegment (x1, y1, x2, y2) =>
                 LineSegment(x1 + deltaX, y1 + deltaY,
                             x2 + deltaX, y2 + deltaY)
             | _ => raise Impossible "Not shiftable"
...
fun preprocess_prog e =
  case e of
       LineSegment (x1, y1, x2, y2) =>
        let
        val xs_close = real_close(x1, x2)
        in
        if xs_close andalso real_close(y1, y2)
        then Point(x1, y1)
        else if xs_close andalso y1 > y2
        then LineSegment(x2, y2, x1, x2)
        else if x1 > x2
